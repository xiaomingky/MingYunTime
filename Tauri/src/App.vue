...
onMounted(() => {
  ...
  if (platformStore.isNetease) {
      // 延迟 1000ms 以等待 Tauri node sidecar (3400端口) 启动就绪
      setTimeout(() => {
          userStore.fetchStatus().then(async (ok) => {
              if (userStore.isLoggedIn && userStore.profile?.userId) {
                  await userStore.syncCloudAccount()
                  await userStore.checkLockStatus()
                  // fetchStatus 失败时（cookie 过期/网络异常），手动加载歌单
                  // 避免"看起来已登录但侧边栏歌单为空"的问题
                  if (!ok && userStore.playlists.length === 0) {
                      userStore.fetchUserPlaylists(userStore.profile.userId)
                  }
              }
          })
          // 启动时静默加载歌单：即使 fetchStatus 还没返回或失败，
          // 如果本地有 cookie 和 profile，立即加载歌单（避免"需要点一下才加载"）
          if (userStore.cookie && userStore.profile?.userId && userStore.playlists.length === 0) {
              userStore.fetchUserPlaylists(userStore.profile.userId)
          }
      }, 1000)
  ...
  if (b && b.on) {
    ...
    // 窗口重新显示时恢复资源（仅当 analyser 被释放时才重建，避免频繁重建 audio graph）
    b.on('window-shown-recover', () => {
        try {
            if (playerStore.audio && !playerStore.analyser && typeof playerStore.rebuildAudioGraph === 'function') {
...
const openOsk = () => {
    const b = getBridge()
    if (b) b.invoke('open-osk')
}

const openAuthorLink = () => {
    const b = getBridge()
    if (b) b.invoke('open-external', { url: import.meta.env.VITE_AUTHOR_URL || '' })
}
const openGithub = () => {
    const b = getBridge()
    if (b) b.invoke('open-external', { url: 'https://github.com/xiaomingky/MingYunTime' })
}

</script>

<template>
  <div class="app-container" :data-platform="platformStore.current">
    <Toast />
    <VideoDownloadToast />
    <ConfirmModal
    ...
    <div class="normal-layout-wrapper">
      <SongDetail />
                ...
                <template v-if="kugouUserStore.isLoggedIn">
                    ...
                    <div v-if="!kugouCreatedCollapsed" class="sidebar-section">
                        <div
                            ...
                            :key="p.id"
                            class="menu-item playlist-item"
                            :class="{ active: String(p.id) === String(kugouUserStore.likedPlaylistId) ? route.path === '/kugou/liked' : route.path === `/kugou/playlist/${p.id}` }"
                            @click="navigateTo(String(p.id) === String(kugouUserStore.likedPlaylistId) ? '/kugou/liked' : `/kugou/playlist/${p.id}`)"
                        >
                            <Heart v-if="String(p.id) === String(kugouUserStore.likedPlaylistId)" :size="16" :fill="'#EC4141'" :color="'#EC4141'" />
