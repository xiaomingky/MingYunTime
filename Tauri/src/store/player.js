...
export const usePlayerStore = defineStore('player', {
    ...
    actions: {
        ...
        async rebuildAudioGraph() {
            // 注意：createMediaElementSource 不能对同一个 HTMLMediaElement 重复调用
            // （即使关闭旧 AudioContext 也不行），因此必须复用已有的 ctx 和 source，
            // 只重建后续节点（analyser / EQ / volumeGain）。
            // resetAudioElement 会重建 audio 元素并把 ctx/source 置空，那时才会创建新 source。

            // 清理后续节点（保留 source 和 ctx）
            ...
            // 首次创建 ctx 和 source（source 只能对每个 audio 元素创建一次）
            if (!this.ctx) {
                try {
                    ...
                    // 降级：直接用 audio.volume 控制音量，不经过 Web Audio API
                    this.audio.volume = Math.min(1, this.volume / 100)
                    return
                }
            }
        ...
        async playSong(song, list = [], options = {}) {
            ...
            try {
                ...
                // 本地歌曲：动态生成协议 URL（避免缓存中 URL 格式过时）
                // Tauri 2 在 Windows/Linux 用 http://local-file.localhost/<path>
                // macOS 用 local-file://<path>
                if (isLocal && song.path) {
                    const bridge = window.__ELECTRON_BRIDGE__ || window.bridge || window.ipcHandler
                    if (bridge && bridge.fileUrl) {
                        url = bridge.fileUrl('local-file', song.path)
                    } else if (!url || !url.startsWith('http://local-file.localhost') && !url.startsWith('local-file://')) {
                        // fallback: 生成 Windows/Linux 格式
                        const encoded = encodeURI(song.path.replace(/\\/g, '/'))
                        url = navigator.userAgent.toLowerCase().includes('mac')
                            ? `local-file://${encoded}`
                            : `http://local-file.localhost/${encoded}`
                    }
                }
                ...
                // 本地歌曲：动态生成封面协议 URL（避免缓存中 URL 格式过时）
                if (isLocal && song.path && normalized.al.picUrl) {
                    const _bridge = window.__ELECTRON_BRIDGE__ || window.bridge || window.ipcHandler
                    const picUrl = normalized.al.picUrl
                    const isOldFormat = picUrl.startsWith('song-cover:///') || picUrl.startsWith('local-file:///')
                    const isNewFormat = picUrl.startsWith('http://song-cover.localhost/') || picUrl.startsWith('http://local-file.localhost/')
                    if (_bridge && _bridge.fileUrl && (isOldFormat || !isNewFormat)) {
                        normalized.al.picUrl = _bridge.fileUrl('song-cover', song.path)
                    } else if (isOldFormat) {
                        // fallback: 生成 Windows/Linux 格式
                        const encoded = encodeURI(song.path.replace(/\\/g, '/'))
                        normalized.al.picUrl = navigator.userAgent.toLowerCase().includes('mac')
                            ? `song-cover://${encoded}`
                            : `http://song-cover.localhost/${encoded}`
                    }
                }

                this.currentSong = normalized
                // Tauri 2 中 local-file 协议(http://local-file.localhost/)是跨域的，
                // createMediaElementSource 要求音频必须 CORS 合规（crossOrigin='anonymous' + 服务器返回 CORS 头），
                // 否则音频数据被"污染"导致静音。因此本地歌曲也需要设置 crossOrigin。
                this.audio.crossOrigin = "anonymous"
