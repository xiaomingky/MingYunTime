...
async function doSearch() {
    ...
    const ipcPromise = (bridge && bridge.searchMultiLyric)
        ? bridge.searchMultiLyric({ songName: songInfo.value.songName, artist })
            ...
            .catch(e => ({ qq: [], kugou: [], errors: { qq: e.message || e, kugou: e.message || e } }))
...
async function selectCandidate(item) {
    ...
    try {
        ...
        console.error('apply candidate err:', e)
        messageStore.error('获取歌词失败: ' + (e?.message || (typeof e === 'string' ? e : JSON.stringify(e)) || '未知错误'))
    } finally {
        fetchingId.value = ''
    }
}
