...
// Twitch iframe 嵌入播放（CDN 403 限制，无法代理分片，使用官方嵌入播放器）
const twitchIframeSrc = ref('')
...
function onVolumeChange() {
    ...
    // 如果有 gainNode，静音切换需要同步到 gainNode
    if (gainNode) {
        gainNode.gain.value = v.muted ? 0 : volume.value * gainBoost.value
    }
}
...
function onVolumeInput() {
    ...
    if (gainNode) {
        ...
        gainNode.gain.value = (volume.value === 0) ? 0 : volume.value * gainBoost.value
    } else {
...
function loadSource(url) {
    ...
    // Twitch 直播：使用官方 iframe 嵌入播放器（CDN 分片受 IP 绑定，无法代理）
    if (url.startsWith('https://www.twitch.tv/')) {
        const channelName = url.replace('https://www.twitch.tv/', '').split('?')[0]
        loading.value = false
        if (channelName) {
            const hn = window.location.hostname || 'localhost'
            twitchIframeSrc.value = `https://player.twitch.tv/?channel=${channelName}&parent=${hn}&parent=localhost&parent=127.0.0.1&muted=false&autoplay=true`
        } else {
            playerError.value = 'Twitch 直播：无法解析频道名'
            emit('error', playerError.value)
        }
        return
    }
    ...
    if (Hls.isSupported()) {
        let playUrl = url
        if (playUrl.includes('ttvnw.net') || playUrl.includes('twitch.tv')) {
            if (!playUrl.startsWith('http://127.0.0.1:3400/proxy/stream')) {
                playUrl = `http://127.0.0.1:3400/proxy/stream?url=${encodeURIComponent(playUrl)}&referer=https://www.twitch.tv/`
            }
        }
        ...
        hls.on(Hls.Events.ERROR, (_, data) => {
            if (data.fatal) {
                if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
                    playerError.value = '网络错误：无法加载视频片段，可能需要代理或不支持跨域'
                    loading.value = false
                    emit('error', playerError.value)
                    destroyHls()
...
<template>
    <div
        ...
        tabindex="0"
    >
        <!-- Twitch 官方 iframe 嵌入播放器 -->
        <iframe
            v-if="twitchIframeSrc"
            :src="twitchIframeSrc"
            class="twitch-iframe"
            allowfullscreen
