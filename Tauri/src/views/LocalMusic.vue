...
const fetchCover = async (song) => {
    ...
    try {
        ...
        if (result.success) {
            song.al.picUrl = bridge.fileUrl ? bridge.fileUrl('song-cover', song.path) : `song-cover:///${encodeURI(song.path.replace(/\\/g, '/'))}`
...
const getCoverUrl = (song) => {
    ...
    // 兼容新旧协议格式：song-cover:/// 和 http://song-cover.localhost/
    const isSongCover = song.al.picUrl.startsWith('song-cover:') || song.al.picUrl.startsWith('http://song-cover.localhost/')
    if (!showGifCover.value && isSongCover) {
        return song.al.picUrl + (song.al.picUrl.includes('?') ? '&' : '?') + 'static=1'
    }
    ...
    try {
        ...
        if (res.success) {
            ...
            if (song) {
                ...
                if (editCover.value) {
                    const b = getBridge()
                    song.al.picUrl = b && b.fileUrl ? b.fileUrl('song-cover', editingSong.value.path) : `song-cover:///${encodeURI(editingSong.value.path.replace(/\\/g, '/'))}`
                }
            }
