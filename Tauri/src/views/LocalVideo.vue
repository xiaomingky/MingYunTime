...
const playVideo = (video) => {
    // 动态生成协议 URL（避免缓存中 URL 格式过时）
    const bridge = window.__ELECTRON_BRIDGE__ || window.bridge || window.ipcHandler
    let videoUrl = video.url
    if (video.path && bridge && bridge.fileUrl) {
        videoUrl = bridge.fileUrl('local-file', video.path)
    } else if (video.path && (!videoUrl || (!videoUrl.startsWith('http://local-file.localhost/') && !videoUrl.startsWith('local-file://')))) {
        const encoded = encodeURI(video.path.replace(/\\/g, '/'))
        videoUrl = navigator.userAgent.toLowerCase().includes('mac')
            ? `local-file://${encoded}`
    ...
    playerStore.currentMvUrl = videoUrl
    playerStore.currentMvId = null
