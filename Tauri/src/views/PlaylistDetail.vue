    ...
    <template v-if="playlist">
        <div class="detail-header">
            <div class="cover-wrapper" :class="{ owner: isOwner }" @click="isOwner && triggerCoverUpload()">
                <div v-if="String(playlist.id) === String(userStore.likedPlaylistId)" class="liked-cover-container" style="width: 100%; height: 100%; background: linear-gradient(135deg, #EC4141, #ff7e7e); display: flex; align-items: center; justify-content: center;">
                    <Heart :size="80" fill="white" color="white" style="filter: drop-shadow(0 2px 8px rgba(0,0,0,0.2));" />
                </div>
                <img v-else :src="playlist.coverImgUrl" class="cover-img" />
