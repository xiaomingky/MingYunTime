...
- **逐词/逐行歌词切换**：支持 YRC 逐词高亮（从左到右流畅过渡）与普通逐行高亮两种模式一键切换
- **逐字动画性能优化**：仅刷新当前激活行的 word，已完成/未开始的 word 自动跳过，DOM 写入量降低 95%+
...
- **三平台并行搜索**：QQ音乐 + 酷狗音乐 + 网易云音乐 同时搜索，每平台最多 30 条结果并列展示
- **逐词歌词支持**：QQ音乐（QRC 解密）、酷狗（KRC 解密）支持逐词歌词，网易云提供 YRC 逐词歌词
- **时长匹配高亮**：与当前歌曲时长差 ≤1 秒的搜索结果自动高亮（红色边框 + 时长徽章），快速定位正确版本
- **带上歌手搜索开关**：可一键切换"带上歌手搜索"和"仅按歌名搜索"，应对同名歌曲场景
- **本地音乐/云音乐通用**：本地音乐和我的云音乐在无 YRC 逐词歌词时自动弹出选择窗口；线上歌曲默认 QQ 匹配 + 网易云回退
- **歌词翻译**：QQ/酷狗/网易云歌词均支持翻译歌词同步显示
